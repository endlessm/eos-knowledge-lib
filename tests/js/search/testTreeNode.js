...
describe('TreeNode to GtkTreeModel converter', function () {
    let model;
    beforeEach(function () {
        model = TreeNode.tree_model_from_tree_node(TEST_OBJ);
    });

    it('creates a tree model with the appropriate columns', function () {
        expect(model.get_n_columns()).toEqual(TreeNode.TreeNodeColumn.NUM_COLUMNS);
        expect([
            TreeNode.TreeNodeColumn.LABEL,
            TreeNode.TreeNodeColumn.INDEX_LABEL,
            TreeNode.TreeNodeColumn.CONTENT
    ...
    it('converts an empty TreeNode', function () {
        ...
        model = TreeNode.tree_model_from_tree_node(EMPTY_OBJ);
    ...
    it('populates the correct fields of the model', function () {
        ...
        let label = model.get_value(iter, TreeNode.TreeNodeColumn.LABEL);
        expect(label).toEqual('Foo');

        let index_label = model.get_value(iter, TreeNode.TreeNodeColumn.INDEX_LABEL);
        expect(index_label).toEqual('1');

        let content = model.get_value(iter, TreeNode.TreeNodeColumn.CONTENT);
    ...
    it('yields the correct tree when the input is not in order', function () {
        ...
        model = TreeNode.tree_model_from_tree_node(reversed_test_obj);
