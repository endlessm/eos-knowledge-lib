      ...
      if (video_sink)
        {
          ...
          if (gtkglsink)
            ...
            {
              GST_WARNING ("Could not create gtkglsink, falling back to gtksink");
              g_clear_object (&video_sink);
            }
        }
      else
        {
          GST_WARNING ("Could not create glsinkbin, falling back to gtksink");
        }
    }
...
on_ekn_media_bin_realize (GtkWidget *widget, EknMediaBin *self)
{
  ...
  /* Create video sink */
  priv->video_sink = ekn_media_bin_video_sink_new (self);
